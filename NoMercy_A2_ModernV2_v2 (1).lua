...
local function __ZiaanHub_Init_Main__()
    ...
    -- ============================================================
    --  MOONWALK LOCK & SWAY (integrated, no second ScreenGui)
    --  The RenderStepped connection only exists while enabled.
    -- ============================================================
    do
        local MoonwalkTab = Window:AddTab({ Name = "Moonwalk", Icon = ICON.User })
        local MoonwalkSection = MoonwalkTab:AddSection({ Name = "Camera Lock & Sway" })
        local Moonwalk = {
            Active = false,
            SwaySpeed = 15,
            Counter = 0,
        ...
        MoonwalkSection:AddButton({
            Name = "Stop Moonwalk",
            Callback = moonwalkStop,
        })
        MoonwalkSection:AddLabel("Render loop aktif hanya saat fitur ON")

        table.insert(ModernWindow.OnDestroyCallbacks, moonwalkStop)
    end
    ...
    function veil_fire()
        ...
        if not targetInfo
        or not targetInfo.Part
        or not targetInfo.Part.Parent
        or not targetInfo.Player
        or not targetInfo.Player.Character
        then
            targetInfo = veil_getClosestSurvivor()
        end
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if VeilState.chargingSpear and VeilConfig.Enabled and VeilConfig.ShowTargetESP and isSpearMode then
            ...
            if target and target.Part and target.Part.Parent then
                VeilState.currentTarget = target
                VeilDraw.Highlight.Parent = target.Part.Parent
                VeilDraw.Highlight.FillColor = Color3.fromRGB(55, 255, 95)
