...
local function toggleInvisibility()
    ...
    if isInvisible then
        ...
        -- 1. Buat klon visual di atas tanah
        ...
        -- Matikan skrip lokal pada klon agar tidak bentrok
        for _, v in pairs(fakeCharacter:GetChildren()) do
            if v:IsA("LocalScript") then v.Disabled = true end
        end

        -- Atur transparansi klon (0.3 agar kamu masih bisa melihat bayangan karaktermu sendiri)
        ...
        -- 2. Pindahkan badan asli ke bawah tanah (Void) agar aman dari player lain
        ...
        -- 3. Sinkronisasi pergerakan secara mulus setiap frame
        ...
        -- Matikan Invisibility (Kembali Normal di tempat terakhir klon berada)
        ...
        if fakeCharacter then
            if cloneRoot and realRoot then
                -- Munculkan badan asli tepat di posisi klon terakhir + sedikit ke atas agar tidak tersangkut lantai
...
-- UI Menu Troller (Tombol Kustom Kamu)
...
memedog.Text = "Fixed Clone Method"
